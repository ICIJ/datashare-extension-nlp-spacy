...
# TODO: avoid duplication and use the same ES client across Tarentula, this repo and
#  the neo4j extension
ASC = "asc"
BOOL = "bool"
COUNT = "count"
...
class ESClient(AsyncElasticsearch):
    ...
    async def count(
        ...
        return await super(ESClient, self).count(
            body=body, index=index, headers=headers, **kwargs
        )
    ...
    async def search(self, body: Optional[Dict], **kwargs):
        ...
        headers = kwargs.pop("headers", dict())
        headers = self._with_headers(headers)
        return await super(ESClient, self).search(
            headers=headers,
            body=body,
